...
def fetch(subjects, action_filter=None, subset=1, parse_3d_poses=True):
    ...
    # ------------------------------Add 3D data into input(2d)---------------------------
    addJoints = [9, 10, 15]  # 9:Head, 10:RWrist, 15:LWrist

    out_poses_2d_added = []
    for out_pose_2d, out_pose_3d in zip(out_poses_2d, out_poses_3d):
        out_pose_2d_added = np.zeros((out_pose_3d.shape[0],out_pose_3d.shape[1],out_pose_2d.shape[2] + out_pose_3d.shape[2]))
        out_pose_2d_added[:,:,0:2] = out_pose_2d

        added3D = np.zeros((out_pose_2d_added.shape[0], out_pose_2d_added.shape[1], 3))
        added3D[:,addJoints,:] = out_pose_3d[:,addJoints,:]

        addNoise = np.random.normal(0, 0.1, (added3D.shape[0], 3, added3D.shape[2]))
        added3D[:, addJoints, :] = added3D[:, addJoints, :] + addNoise

        out_pose_2d_added[:, :, 2:5] = added3D
        out_poses_2d_added.append(out_pose_2d_added)

    out_poses_2d = out_poses_2d_added
    # -----------------------------------------------------------------------------------
...
if args.render:
    ...
    def fetch_actions(actions):
        ...
        out_poses_2d_added = []
        for out_pose_2d, out_pose_3d in zip(out_poses_2d, out_poses_3d):
            out_pose_2d_added = np.zeros(
                (out_pose_3d.shape[0], out_pose_3d.shape[1], out_pose_2d.shape[2] + out_pose_3d.shape[2]))
            out_pose_2d_added[:, :, 0:2] = out_pose_2d
